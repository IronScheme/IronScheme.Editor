...
using Utility = IronScheme.Editor.Runtime.Compression;
...
//        Trace.WriteLine(o.Fullname);
    ...
    public void LoadAssemblies(params string[] assnames)
    {
      if (assnames.Length > 0)
      {
        ...
        AssemblyLoader aa = assloader.CreateInstanceAndUnwrap("xacc", "IronScheme.Editor.Build.Project+AssemblyLoader") as AssemblyLoader;
...
%(AccessedTime) Contains the timestamp from the last time the time was accessed.
    ...
    internal void ExistingFile(object sender, EventArgs e)
    {
      ...
      //{
      //  string[] extss = IronScheme.Editor.Build.InputExtensionAttribute.GetExtensions(act);
