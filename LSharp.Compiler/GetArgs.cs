  ...
  public class DefaultArgAttribute : ArgItemAttribute
  {
    ...
    public void PrintHelp()
    {
      ...
      if (acceptsdefault)
      {
		...
		static bool casesensitive, allowshortcut, pauserr, ignoreunknown;
    ...
    readonly Hashtable argz = new Hashtable();
    readonly AssemblyName assname;
...
(?<value>\S+)))*										# any single value",						
      ...
      if (allargs.IndexOf(prefix + "nologo") < 0)
      {
        ...
        AssemblyDescriptionAttribute adesc = null;

        atts = ass.GetCustomAttributes(typeof(AssemblyDescriptionAttribute), false);
        if (atts.Length > 0)
        {
          adesc = atts[0] as AssemblyDescriptionAttribute;
        }

        string progname = string.Format(@"{0} {1} {3}
{2}", assname.Name, assname.Version,
          acopy == null ? string.Empty : acopy.Copyright, 
          adesc == null ? string.Empty : (" - " + adesc.Description));

        Console.WriteLine(progname);
      }
      else
      {
        allargs = allargs.Replace(prefix + "nologo", string.Empty);
      }

      defvals = null;

			if (allargs.IndexOf(prefix + "?") >= 0 || allargs.IndexOf(prefix + "help") >= 0)
			{
				PrintHelp(argz);
				Environment.Exit(0);
			}
